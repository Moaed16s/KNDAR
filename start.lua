...
print("::KNDAR::")
local RunBot = io.open("KNDAR", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/KNDAR
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr KNDAR.lua
wget "https://raw.githubusercontent.com/Moaed16s/KNDAR/master/KNDAR.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./KNDAR.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S KNDAR -X kill
screen -S KNDAR ./KNDAR
done
]])
RunTs:close()
end
